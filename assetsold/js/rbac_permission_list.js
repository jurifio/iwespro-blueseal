(function ($) {
    const treeSelector = "#permissionTree";
    $(document).ready(function () {
        ...
        $(treeSelector).fancytree({
            ...
            checkbox: false,
            source: {
                url: "/blueseal/xhr/RbacPermissionTreeController"
            },
            dnd: {
                autoExpandMS: 400,
                focusOnClick: true,
    ...
    $(document).on('bs.permission.insert', function () {
        var tree = $(treeSelector).fancytree("getTree"),
            node = tree.getActiveNode();
        if (!node) node = tree.getRootNode();
        var bsModal = new $.bsModal('Aggiungi permesso', {
            body: '<p>Il nuovo nodo verrà inserito come figlio di ' + node.title + '</p>' +
            '<div class="form-group form-group-default required">' +
            '<label for="new-permission-name">Nome</label>' +
            '<input autocomplete="off" id="new-permission-name" class="form-control" name="new-permission-name" required>' +
            '</div>' +
            '<div class="form-group form-group-default required">' +
            '<label for="new-permission-description">Descrizione</label>' +
            '<input autocomplete="off" id="new-permission-description" class="form-control" name="new-permission-description">' +
            '</div>'
        });
        bsModal.showCancelBtn();
        bsModal.setOkEvent(function () {
            var input = $('input#new-permission-name').val(),
                description = $('input#new-permission-description').val();
