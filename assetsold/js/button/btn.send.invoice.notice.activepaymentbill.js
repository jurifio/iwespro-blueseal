window.buttonSetup = {
    ...
    rel: "tooltip",
    title: "Manda Avvisi di mancato Pagamento Distinte",
    placement: "bottom",
    event: "btn-send-invoice-notice-activepaymentbill"
};

$(document).on('btn-send-invoice-notice-activepaymentbill', function () {
    "use strict";
    ...
    if (selectedRows.length === 1) {
        ...
        bsModal.setOkEvent(function () {
            ...
            if($('#invoiceOption').val() === "invia") {
                $.ajax({
                    method: 'post',
                    url: '/blueseal/xhr/GenerateNoticeActivePaymentSlipMailAjaxController',
                    data: {
                        id: id
                    }
                }).done(function (res) {
                    bsModal.writeBody('Sollecit inviata con successo');
                }).fail(function (res) {
