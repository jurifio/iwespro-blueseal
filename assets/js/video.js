...
var decodeCallback = function (ptr, len, resultIndex, resultCount) {
    ...
    $.ajax({
        url: '/blueseal/xhr/GetProductByEanAjaxController',
        type: "GET",
        data: {
            search: String.fromCharCode.apply(null, result)
        },
        dataType: 'json'
    }).done(function (res) {
        let rawProduct = res;
        ...
        $.each(rawProduct, function (k, v) {
           ...
           $('#resultCPF').val(v.resultCPF);
           $('#resultVariante').val(v.resultVariante);
            $('#resultBrand').val(v.resultBrand);
            $('#id').val(v.id);
            $('#productVariantId').val(v.productVariantId);
