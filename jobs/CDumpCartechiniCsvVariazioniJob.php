...
WHERE ds.qty > 0  and ds.status='ok' AND ds.`checksum` IS NOT null  AND p.productSeasonId >41  AND pdt.langId=1 AND s.id=1 AND CONCAT(st.sigla, ' ',st.address,',',st.`number`,' ',`st`.`city`,' ',`st`.`phone`)  IS  not null AND
...
#GROUP BY dp.productId,dp.productVariantId,ds.productSizeId,ds.storeHouseId,`Custom Column 1`
