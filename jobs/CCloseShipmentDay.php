                ...
                if ($shipment->scope == CShipment::SCOPE_US_TO_USER) {
                    ...
                    foreach ($shipment->order as $order) {
                        ...
                        try {
                            ...
                            $emailRepo->newPackagedTemplateMail('shipmentclient', 'no-reply@iwes.pro', $to, [], [], ['order' => $order, 'coupon' => $coupon, 'orderId' => $order->id, 'shipment' => $shipment, 'lang' => $order->user->lang]);
