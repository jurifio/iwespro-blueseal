...
class CPrepareProductForMarketplaceJob extends ACronJob
{
 ...
 and p.productVariantId=shp.productVariantId where p.qty > 0 and p.id > 190000 and p.productStatusId in (6) and p.productSeasonId > 35 and  shp.shopId ='.$shop->id.') UNION
(select p2.id as productId, p2.productVariantId as productVariantId, p2.qty as qty, shp2.shopIdDestination as shopId from
 ...
 and p2.productVariantId=shp2.productVariantId where p2.qty > 0 and p2.id > 190000 and p2.productStatusId in (6) and p2.productSeasonId > 35 and  shp2.shopIdDestination ='.$shop->id.')';
