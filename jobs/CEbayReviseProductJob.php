 ...
 * @since 1.0
 */
class CEbayReviseProductJob extends ACronJob
{
    ...
    private function ReviseProductsInEbay()
    {
        ...
        foreach ($marketplaceHasShop as $marketplace) {
            ...
            $getMarketplaceShop = $db_con->prepare('select conf_value as marketplaceId from ps_fastbay1_shop_marketplace where conf_key="mp_select" and id_shop=' . $marketplace['prestashopId']);
            $getMarketplaceShop->execute();


            foreach ($getMarketplaceShop as $market) {
                try {
                    ...
                    $rowCountryShop = $getMarketplaceShop->fetchAll(PDO::FETCH_ASSOC);
                    ...
                    $shop = $shopRepo->findOneBy(['id' => $market['shopId']]);
...
<ReviseFixedPriceItem xmlns="urn:ebay:apis:eBLBaseComponents">
  ...
  <WarningLevel>High</WarningLevel>';
    ...
    <PostalCode>' . $rowZipCodeShop[0]['shop_zip_code'] . '</PostalCode>
    <Location>' . $rowCompanyCity[0]['company_city'] . '</Location>
    <BestOfferDetails>
    ...
    id_shop=' . $marketplace['prestashopId'] . ' and id_marketplace="' . $market['marketplaceId'] . '" limit 1');
        ...
        <SellerShippingProfile> 
         ...
         <ShippingProfileName>' . $rowDescShippingRules['id_profile'] . '</ShippingProfileName>
                    ...
                    $getToken = $db_con->prepare('select token from ps_fatbay1_token where id_shop=' . $marketplace['prestashopId'] . ' and id_marketplace="' . $market['marketplaceId'] . '" limit 1');
                    $getToken->execute();
                    $rowGetToken = $getToken->fetchAll(PDO::FETCH_ASSOC);


                    $xml .= '<RequesterCredentials>
    <eBayAuthToken>' . $rowGetToken['token'] . '</eBayAuthToken>
...
// Send the Request
        ...
        $this->report('CEbayReviseProductJob','End Export to Ebay');
    }
}