...
/**
 * Class CVatRegisterListAjaxController
 ...
 * @since ${VERSION}
 */
class CVatRegisterListAjaxController extends AAjaxController
{
    public function get()
    {
        $sql = "SELECT
                ...
                WHERE `o`.`status` LIKE 'ORD%' and `i`.`invoiceNumber` is not null  GROUP BY ol.id, ol.orderId";
        ...
        foreach ($orders as $val) {
            ...
            $invoice=$invoiceRepo->findOneBy(['orderId'=>$val->id]);
            ...
            if($invoice->printSummaryDate!=null){
                $row['printed']='Stampato  in Data' . $invoice->printSummaryDate;

            }else{
                $row['printed']='Non Stampato';
            }
