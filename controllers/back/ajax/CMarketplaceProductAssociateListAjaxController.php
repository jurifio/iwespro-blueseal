...
class CMarketplaceProductAssociateListAjaxController extends AAjaxController
{
    public function get()
    {
       ...
       '' as   prestashopProductId,
        
       '' as TypeRetouchPrice,
       '' as marketplaceHasShopId,
      '' as priceMarketplace,
       '' as isOnSale,
       '' as priceSale,
       '' as typeSale,
...
where (`p`.`qty` > 0) and (p.productStatusId =6) or (p.productStatusId=11)
        ...
        foreach ($datatable->getResponseSetData() as $key => $row) {
           $resproductFindEan=\Monkey::app()->repoFactory->create('ProductEan')->findOneBy(['productId'=>$row['productId'],'productVariantId'=>$row['productVariantId'],'productSizeId'=>'0','used'=>'1','usedForParent'=>'1'])  ;
            ...
            if(null == $resmarketplaceHasProductAssociate){
                ...
                $row['prestashopProductId']="non presente";

            }else{
                $resmarketplacearray=$this->app->dbAdapter->query("SELECT mphpa.id as idpresta, m.name as name,s.name as nameShop, mphpa.typeRetouchPrice as typeRetouchPrice, mphpa.amount as amount,mphpa.price as price, 
                             ...
                             mphpa.titleTextSale as titleTextSale
                ...
                $row['prestashopProductId']=$marketplaces['idpresta'];
                $row['titleTextSale']=$marketplaces['titleTextSale'];
                $row['priceSale']=$marketplaces['priceSale'];
