 ...
 * @since ${VERSION}
 */
class CProductTagExclusiveListAjaxController extends AAjaxController
{
    protected $urls = [];
    ...
    public function get()
    {
        $sql = "SELECT
                ...
                FROM ((((((((((`Product` `p`
                  ...
                  `ProductHasTagExclusive` `pht`
                    ON (((`pht`.`productId` = `p`.`id`) AND (`pht`.`productVariantId` = `p`.`productVariantId`)))) LEFT JOIN `TagExclusive` `t`
                    ON ((`pht`.`tagExclusiveId` = `t`.`id`))) LEFT JOIN
                  `ProductColorGroup` `pcg` ON ((`p`.`productColorGroupId` = `pcg`.`id`))
                  LEFT JOIN ProductColorGroupTranslation T2 ON pcg.id = T2.productColorGroupId)
                WHERE ((`T2`.`langId` = 1) AND (`ps`.`code` IN ('A', 'P', 'I')))
        ...
        foreach($prodotti as $val){
	        ...
	        $tags = [];
	        foreach ($val->tagExclusive as $tag) $tags[] = $tag->getLocalizedName();
