...
class CProductNamesListAjaxController extends AAjaxController
{
    public function get()
    {
      ...
      from (((((`ProductName` `pn` 
          ...
          join `ProductCategory` `pc` on((`phpc`.`productCategoryId` = `pc`.`id`))) on(((`p`.`id` = `phpc`.`productId`) and (`p`.`productVariantId` = `phpc`.`productVariantId`)))) 
          join `ProductStatus` on((`ProductStatus`.`id` = `p`.`productStatusId`)))
          where ((`pn`.`langId` = 1) and (`pnt`.`langId` = 1) and (`ps`.`stockQty` > 0) and (not((`p`.`dummyPicture` like '%bs-dummy%'))) and (`p`.`productStatusId` in (5,6,11)))";
