...
class CDictionaryRemasterImageRenameToMetaAjaxController extends AAjaxController
{
...
//nome file elaborato
                    ...
                    $firstPosition = strpos($filenametoextrat,'_');

                    $lastPosition = strpos($filenametoextrat,substr(strrchr($filenametoextrat,'__'),0));

                    $firstPart = substr($filenametoextrat,0,$firstPosition);
                    $lastPart=substr($filenametoextrat,$firstPosition+1,8);
                    $prod=explode('-',$firstPart);
                    $product=\Monkey::app()->repoFactory->create('Product')->findOneBy(['id'=>$prod[0],'productVariantId'=>$prod[1]]);
$findDirtyProductInt=\Monkey::app()->repoFactory->create('DirtyProduct')->findOneBy(['productId'=>$prod[0],'productVariantId'=>$prod[1]]);
$dirtyProductId = $findDirtyProductInt->id;
$findBarcodeInt=\Monkey::app()->repoFactory->create('DirtySku')->findOneBy(['dirtyProductId'=>$dirtyProductId]);
$productBrand=\Monkey::app()->repoFactory->create('ProductBrand')->findOneBy(['id'=>$product->productBrandId]);
if($productBrand->name!=null){
...
$productVariant=\Monkey::app()->repoFactory->create('ProductVariant')->findOneBY(['id'=>$prod[1]]);
if($productVariant!=null){
    $productVariantName= ' - '.$productVariant->name;
}else{
    $productVariantName='';
}
if($findBarcodeInt!=null) {
    $barcodeInt = $findBarcodeInt->barcode;

    if (empty($barcodeInt)) {
        $prefinalName = $firstPart . '__' . $productBrandName  . $product->itemno .  $productVariantName . '_' . $lastPart;
    } else {
        $prefinalName = $firstPart . '__' . $productBrandName . ' - ' . $product->itemno .  $productVariantName . '__' . $barcodeInt . '_' . $lastPart;
    }
}else{
    $prefinalName = $firstPart . '__' . $productBrandName . ' - ' . $product->itemno . ' - ' . $productVariantName . '_' . $lastPart;
}
