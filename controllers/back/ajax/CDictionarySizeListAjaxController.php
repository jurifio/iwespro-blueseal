 ...
 * @since ${VERSION}
 */
class CDictionarySizeListAjaxController extends AAjaxController
{
    protected $urls = [];
    ...
    public function get()
    {
        $editShopLink = $this->urls['base']."importatori/dizionari/taglie/modifica";
  ...
  (SELECT count(0) AS `count`
   FROM `DictionarySize` `d2`
   WHERE ((`d2`.`shopId` = `d1`.`shopId`) AND isnull(`d2`.`productSizeId`))) AS `mancanti`
FROM `DictionarySize` `d1` join Shop s on `d1`.`shopId`  = s.id WHERE s.isVisible='1'
GROUP BY `d1`.`shopId`";
        ...
        foreach($shops as $shop) {
            ...
            $response['data'][$i]['shopId'] = '<a data-toggle="tooltip" title="traduci taglia" data-placement="right" href="'.$editShopLink.'/'.$shop->id.'" >'.$shop->title.'</a>';
