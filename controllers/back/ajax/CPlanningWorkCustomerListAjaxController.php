    ...
    public function get()
    {
        ...
        if($userHasShop){
            ...
            if($brca){
                ...
                $sqlFilter=' where t1.billRegistryClientId='.$billRegistryclientid. ' ';
            }
        }
        $sql = 'SELECT
                      t1.id as id,
       ...
       `brc`.`companyName` as `companyName`       
                    ...
                    join PlanningWorkType pt on t1.planningWorkTypeId=pt.id '.$sqlFilter.' order by t1.startDateWork DESC';
