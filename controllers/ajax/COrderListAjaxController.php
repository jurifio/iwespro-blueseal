...
class COrderListAjaxController extends AAjaxController
{
    public function get()
    {
        ...
        if ($critical) {
            $sql .= " AND ((`o`.`status` LIKE 'ORD_PENDING' AND `ol`.`status` NOT LIKE 'ORD_FRND_CANC' AND (`o`.`orderPaymentMethodId` <> 5 OR `o`.`paymentDate` is NULL) ) " .
                " OR (`ol`.`status` LIKE 'ORD_FRND_OK' AND `os`.`id` NOT IN (8,9,10) AND `ols`.`id` < 8) AND `ol`.`status` NOT LIKE 'ORD_CANCEL' AND `ol`.`status` NOT LIKE 'ORD_RETURNED' and `o`.`paymentDate` is NULL)";
        }
