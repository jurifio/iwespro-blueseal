...
/**
 * Class CDictionaryCategoryListAjaxController
 ...
 * @since ${VERSION}
 */
class CDictionaryCategoryListAjaxController extends AAjaxController
{
    protected $urls = [];
    ...
    public function get()
    {
        $editShopLink = $this->urls['base']."importatori/dizionari/categorie/modifica";
        $datatable = new CDataTables('vBluesealDictionaryCategoryList',['id'],$_GET);
        ...
        foreach($shops as $shop) {
            ...
            $res2 = $this->app->dbAdapter->query("SELECT count(0) AS mancanti
                                                FROM DictionaryCategory d1
                                                WHERE (d1.shopId = ? AND isnull(d1.productCategoryId))", [$shop->id])->fetchAll();

            $response['data'][$i]["DT_RowId"] = 'row__'.$shop->id;
            $response['data'][$i]["DT_RowClass"] = 'colore';
            $response['data'][$i]['shopId'] = '<a data-toggle="tooltip" title="traduci categoria" data-placement="right" href="'.$editShopLink.'/'.$shop->id.'" >'.$shop->title.'</a>';
