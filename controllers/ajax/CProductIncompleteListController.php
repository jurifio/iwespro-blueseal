...
class CProductIncompleteListController extends AAjaxController
{
    ...
    public function get()
    {
        ...
        $sql = "select `p`.`id` AS `id`,`p`.`productVariantId` AS `productVariantId`,`sp`.`shopId` AS `shopId`,`s`.`name` AS `shop`,concat(`p`.`itemno`,' # ',`pv`.`name`) AS `code`,`p`.`productSeasonId` AS `season`,`p`.`externalId` AS `externalId`,`pb`.`name` AS `brand`,sum(`pk`.`stockQty`) AS `totalQty`,`ps`.`name` AS `status`,`p`.`creationDate` AS `creationDate`,concat_ws(' ',if(isnull(`p`.`productSizeGroupId`),'E400',NULL),if(isnull(`p`.`productSeasonId`),'E410',NULL),if((`p`.`productBrandId` = 1),'E420',NULL),if(isnull(`p`.`productSheetPrototypeId`),'E430',NULL),if(isnull(`pc`.`productColorGroupId`),'E440',NULL),if(isnull(`pp`.`productId`),'E450',NULL),if(isnull(`pk`.`productId`),'E460',NULL),if(isnull(`pn`.`name`),'E470',NULL),if(isnull(`pd`.`productId`),'E480',NULL)) AS `problems` from ((((((((((`Product` `p` left join `ShopHasProduct` `sp` on(((`p`.`id` = `sp`.`productId`) and (`p`.`productVariantId` = `sp`.`productVariantId`)))) left join `ProductHasProductColorGroup` `pc` on(((`pc`.`productId` = `p`.`id`) and (`pc`.`productVariantId` = `p`.`productVariantId`)))) left join `ProductNameTranslation` `pn` on(((`pn`.`productId` = `p`.`id`) and (`pn`.`productVariantId` = `p`.`productVariantId`)))) left join `ProductBrand` `pb` on((`p`.`productBrandId` = `pb`.`id`))) left join `Shop` `s` on((`s`.`id` = `sp`.`shopId`))) left join `ProductVariant` `pv` on((`p`.`productVariantId` = `pv`.`id`))) left join `ProductStatus` `ps` on((convert(`ps`.`id` using utf8) = `p`.`productStatusId`))) left join `ProductSku` `pk` on(((`p`.`id` = `pk`.`productId`) and (`p`.`productVariantId` = `pk`.`productVariantId`)))) left join `ProductHasProductPhoto` `pp` on(((`p`.`id` = `pp`.`productId`) and (`p`.`productVariantId` = `pp`.`productVariantId`)))) left join `ProductDescription` `pd` on(((`p`.`id` = `pd`.`productId`) and (`p`.`productVariantId` = `pd`.`productVariantId`)))) where ((`ps`.`code` not in ('C','N')) and (`pn`.`langId` = 1) and (`pn`.`name` = '') and (isnull(`pc`.`productColorGroupId`) or isnull(`p`.`productSizeGroupId`) or isnull(`p`.`productSeasonId`) or (`p`.`productBrandId` = 1) or isnull(`p`.`productSheetPrototypeId`) or ((`ps`.`code` in ('W','G')) and (`p`.`dummyPicture` <> '0000000000.jpg')))) group by `p`.`id`,`p`.`productVariantId` having (`totalQty` > 0) order by `p`.`creationDate` desc";
