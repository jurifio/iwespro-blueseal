...
/**
 * Class CAggregatorCategoryAssignController
 ...
 * @since 1.0
 */
class CAggregatorCategoryAssignController extends ARestrictedAccessRootController
{
    ...
    public function get()
    {
        ...
        $view->setTemplatePath($this->app->rootPath().$this->app->cfg()->fetch('paths','blueseal').'/template/aggregator_category_assign.php');
	...
	public function put()
	{
		$sample = \Monkey::app()->repoFactory->create('MarketplaceAccountCategory')->getEmptyEntity();
		$sample->readId($this->app->router->request()->getRequestData("id"));
		$one = \Monkey::app()->repoFactory->create('MarketplaceAccountCategory')->findOneBy($sample->getIds());
