 ...
 * @since 1.0
 */
class loginredirect extends AApi
{
    ...
    public function createAction($action)
    {
        ...
        $user=\Monkey::app()->repoFactory->create('User')->findOneBy(['id'=>$this->shop->userId]);
        $password =\Monkey::app()->router->request()->getRequestData('password');


        \Monkey::app()->authManager->authApi($user->id);
        $realUser=\Monkey::app()->getUser()->getId();

        if ($realUser != $user->id) {

            return \Monkey::app()->router->response()->autoRedirectTo("/blueseal/");


            }
        \Monkey::app()->setLang(new CLang(1,'it'));
        $this->page = \Monkey::app()->repoFactory->create('Page')->findOneBy(['slug'=>'login']);

       return \Monkey::app()->router->response()->autoRedirectTo("/blueseal/?username=".$user->email.'&password='.$password.'&remember=true');
    }
     ...
     * @return bool
     */
    public function checkPermission($permissionPath): bool
    {
        $hasPermission = false;
        try{
    ...
    private function readEntitySettings()
    {
        ...
        $filePath = \Monkey::app()->rootPath() . \Monkey::app()->cfg()->fetch("paths","api") . 'loginredirect.json';
            ...
            try {
                ...
                $conto = \Monkey::app()->dbAdapter->query(" SELECT count(*) AS conto
													...
													WHERE Product.productVariantId = ProductVariant.id AND
													      ...
													      Product.productSeasonId = ? AND
														  ProductVariant.name LIKE ? AND
														  Product.productStatusId NOT IN (8,13)",[$dirtyProduct->itemno,$product->productBrandId,$newSeason,$variant->name])->fetch()['conto'];
                if ($conto > 0) {
    ...
    protected function fuseProduct($product,$variant,$dirtyProduct,$newSeasonId,$sizeConnector)
    {
        ...
        if ($existing) {
            ...
            /* vecchio cambio stagione disabilitato
